...
raw_material_cost = (fix_H2_price*double(sol.F2_H2)*mol_wt_H2 + fix_CO2_price*double(F1_CO2)*mol_wt_CO2)*ophours;

xcat = categorical({'With recycle', 'Without recycle'});

%% first row represents with recyle %%
%% second row represents without recycle %% 
%% stacked bar graph showing cost contributions per tonne of methanol produced %% 
